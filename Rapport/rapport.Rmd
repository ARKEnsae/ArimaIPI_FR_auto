...
X_T+\hat \theta_1\varepsilon_T -q_{1-\frac \alpha 2}{\sigma}_h\;;\;
X_T+\hat \theta_1\varepsilon_T +q_{1-\frac \alpha 2}{\sigma}_h
\right]
(\#eq:icCourtTerme)
\end{equation}
Le problème est que $\sigma_h$ et $\theta_1$ sont inconnus. Si $\theta_1$ est connu on peut estimer $\sigma_h$ par $\tilde \sigma_h$ avec :
$$
\tilde\sigma_1 = \tilde\sigma=  \frac{1}{T-2}\sum_{t=2}^T\hat\varepsilon_t^2
\text{ et }\quad
\tilde\sigma_2=\hat\sigma\sqrt{1+(1+ \theta_1)^2}
$$
L'intervalle de confiance \@ref(eq:icCourtTerme) reste valide en remplaçant $\sigma_h$ par $\tilde \sigma_h$ et $q_{1-\frac \alpha 2}$ par le quantile $1-\frac \alpha 2$ d'une loi de Student de degré $T-2$. Dans notre cas, puisque $\theta_1$ n'est pas connu on le remplace dans les formules précédentes par $\hat \theta_1$, qui est l'estimation que l'on fait à partir de nos données. On estime donc $\sigma_h$ par $\hat \sigma_h$ avec :
$$
...
$$
En remplaçant $\sigma_h$ par $\hat\sigma_h$, $\theta_1$ par $\theta_2$, les intervalles de confiance définis dans l'équation \@ref(eq:icCourtTerme) restent valides mais **asymptotiquement uniquement**. 
...
Pour obtenir ces intervalles de confiance il faut :
...
- que le modèle suivit par notre série entre janvier 2010 et février 2020 soit bien un modèle ARIMA(0,1,1).
