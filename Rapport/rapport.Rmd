...
On cherche à construire un intervalle confiance de niveau $\alpha$ de $X_{T+h\vert T}$ pour $t \in \{1,2\}$ : 

$$
%P_{(Ho)} (X_{T+h\vert T} \in IC_{1-\alpha}   ) = 
P_{(Ho)} (\lvert \hat \varepsilon_{T+2\vert T} \rvert \leq q_{1-\frac \alpha 2}  ) = 1-\alpha
\iff 
IC_{1-\alpha}(X_{T+h\vert T}) =
\left[
...
Avec $(Ho) \: \hat\theta_1 = \theta_1$, $q_{1-\frac \alpha 2}$ le quantile $1-\frac \alpha 2$ d'une loi $\mathcal N(0,1)$ 

En effet, les prévisions de $X_{T+1}$ et $X_{T+2}$ réalisées à l'instant $T$, notées $\hat X_{T+1\vert T}$ et $\hat X_{T+2\vert T}$, vérifient  l'équation :
$$
\begin{cases}
...
\end{cases}
$$
Les erreurs de prévision sont alors égales à :
$$
\begin{cases}
...
\iff 
\begin{cases}
\hat \varepsilon_{T+1\vert T}  \sim \mathcal N(0,{\sigma_1}^2)\text{ avec } \sigma_1^2=\sigma^2
\\
\hat \varepsilon_{T+2\vert T} \sim \mathcal N(0,{\sigma_2}^2) \text{ avec } \sigma_2^2 = {\sigma}^2 (1+\theta_1)^2
\end{cases}
$$
