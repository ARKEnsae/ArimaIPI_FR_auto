...
[*Question 6*] En somme, la région de confiance pour $(X_{T+1},X_{T+2})$ est une ellipse, dont le centre est $(\widehat X_{T+1\vert T}, \widehat X_{T+2\vert T})$ et caractérisé par l'équation :
...
L'application numérique donne $\widehat X_{T+1\vert T}= \widehat X_{T+2\vert T}\simeq103,7$ et pour $\alpha = 0,05$ :
\begin{equation}
R_{95\%}=\left\{ 
\begin{pmatrix}
	x \\ y
...
La forme allongée de la région de confiance reflète une certaine cohérence entre les prévisions de $X_{T+1}$ et $X_{T+2}$ (que l'on a pas quand on construit des intervalles de confiance de manière indépendante). 
En effet, plus  $\widehat X_{T+1\vert T}$, plus les valeurs « plausibles » de $\widehat X_{T+2\vert T}$ sont élevées.
Par ailleurs, le grand axe de l'ellipse (plus long diamètre de l'ellipse) n'est pas sur la première : cela reflète que l'on ne peut avoir de « trop » grandes (ou « trop » petites) valeurs à la fois pour $\widehat X_{T+1\vert T}$ et $\widehat X_{T+2\vert T}$.
