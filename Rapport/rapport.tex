...
\CommentTok{# que les résidus soient indépendants (on ne veut plus d'endogénéité dû aux variables}
\CommentTok{# omises)}
...
\NormalTok{        pval <-}\StringTok{ }\ControlFlowTok{if}\NormalTok{ (l}\OperatorTok{<=}\NormalTok{fitdf) }\OtherTok{NA} \ControlFlowTok{else} \KeywordTok{Box.test}\NormalTok{(series, }\DataTypeTok{lag=}\NormalTok{l, }\DataTypeTok{type=}\StringTok{"Ljung-Box"}\NormalTok{,}
                                               \DataTypeTok{fitdf=}\NormalTok{fitdf)}\OperatorTok{$}\NormalTok{p.value }
...
\CommentTok{# Grâce à la fonction evaluation_model, on repère les modèles}
\CommentTok{# possibles qui vérifient deux conditions : }
...
\NormalTok{## Pour éviter de tout écrire à la main :}
...
\CommentTok{# On regarde par curiosité les critères d'information}
...
\NormalTok{ordres_retenus <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{,}\DecValTok{1}\NormalTok{,}\DecValTok{1}\NormalTok{) }\CommentTok{#sur la série initiale : d = 1 }
\CommentTok{# (on l'a différenciée une fois) et q=1 (MA(1)) }
...
\CommentTok{#Modèle bien valide : on remarque que aucun ordre de lag reste significativement non nul}
\CommentTok{# dans ACF/PACF}
