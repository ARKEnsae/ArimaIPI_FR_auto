...
\CommentTok{# considérons, comme dans les TD, qu'il y a ici pas de tendance et}
...
\CommentTok{# On trouve un lag de deux :}
...
\NormalTok{adf }\CommentTok{# on ne rejette pas : série non stationnaire avec une racine unitaire}
...
\KeywordTok{PP.test}\NormalTok{(x) }\CommentTok{# il y a une racine unitaire}
\NormalTok{tseries}\OperatorTok{::}\KeywordTok{kpss.test}\NormalTok{(x) }\CommentTok{# série non stationnaire}
...
\KeywordTok{adfTest_valid}\NormalTok{(x_st, }\DataTypeTok{kmax =} \DecValTok{24}\NormalTok{, }\DataTypeTok{type =} \StringTok{"nc"}\NormalTok{)}
