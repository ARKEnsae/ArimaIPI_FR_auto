...
models_possibles <- expand.grid(p = 0:3, d = 0, q = 0:4)
...
# Il y a pas indépendance des résidus mais coefs MA non significatifs
# Remarquons que les résidus ne sont pas homoscédastiques : a priori on ne peut 
# pas interpréter la significativité (ce qui est significatif pourrait ne pas l'être)
# mais on n'a pas vu ça en cours
...
# Il n'y a pas indépendance des résidus
models_evalues$`ARIMA(0,0,4)`
# Modèle a priori OK : coefficients significatifs et résidus indépendants
# Remarquons que les résidus ne sont pas homoscédastiques : a priori on ne peut 
# pas interpréter la significativité (ce qui est significatif pourrait ne pas l'être)
# mais on n'a pas vu ça en cours
models_evalues$`ARIMA(1,0,4)`
# Il y a pas indépendance des résidus mais coefs MA(4) non significatif
models_evalues$`ARIMA(2,0,4)`
# Il y a pas indépendance des résidus mais aucun coef significatif
models_evalues$`ARIMA(3,0,4)`
# Il y a pas indépendance des résidus mais aucun coef significatif

# Bilan : seul modèle vaile : ARIMA(0,0,4)

ordres_retenus <- c(0,1,4)
...
#Remarquons que le modèle qui serait déterminé automatiquement ne serait pas le même
# en effet dans celui-ci on ne fait pas des tests d'indépendance :
